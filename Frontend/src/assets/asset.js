import img1 from "./1.jpg"
import img2 from "./2.jpg"
import img3 from "./3.jpg"
import img4 from "./4.jpg"
import img5 from "./5.jpg"
import img6 from "./6.jpg"
import img7 from "./7.jpg"
import img8 from "./8.jpg"
import img9 from "./9.jpg"
import img10 from "./10.jpg"
import img11 from "./11.jpg"
import img12 from "./12.jpg"
import img13 from "./13.jpg"
import img14 from "./14.jpg"
import img15 from "./15.jpg"
import img16 from "./16.jpg"
import img17 from "./17.jpg"
import img18 from "./18.jpg"
import img19 from "./19.jpg"
import img20 from "./20.jpg"
import img21 from "./21.jpg"
import img22 from "./22.jpg"
import img23 from "./23.jpg"
import img24 from "./24.jpg"
import img25 from "./25.jpg"
import img26 from "./26.jpg"
import img27 from "./27.jpg"
import img28 from "./28.jpg"
import img29 from "./29.jpg"
import img30 from "./30.jpg"
import img31 from "./31.jpg"
import img32 from "./32.jpg"
import img33 from "./33.jpg"
import img34 from "./34.jpg"
import img35 from "./35.jpg"
import img36 from "./36.jpg"
import img37 from "./37.jpg"
import img38 from "./38.jpg"
import img39 from "./39.jpg"
import img40 from "./40.jpg"
import img41 from "./41.jpg"
import img42 from "./42.jpg"
import img43 from "./43.jpg"
import img44 from "./44.jpg"
import img45 from "./45.jpg"
import img46 from "./46.jpg"
import img47 from "./47.jpg"
import img48 from "./48.jpg"
import img49 from "./49.jpg"
import img50 from "./50.jpg"
import img51 from "./51.jpg"
import img52 from "./52.jpg"
import img53 from "./53.jpg"
import img54 from "./54.jpg"
import img55 from "./55.jpg"
import img56 from "./56.jpg"
import img57 from "./57.jpg"
import img58 from "./58.jpg"
import img59 from "./59.jpg"
import img60 from "./60.jpg"
import img61 from "./61.jpg"
import img62 from "./62.jpg"
import img63 from "./63.jpg"
import img64 from "./64.jpg"
import img65 from "./65.jpg"
import img66 from "./66.jpg"
import img67 from "./67.jpg"
import img68 from "./68.jpg"
import img69 from "./69.jpg"
import img70 from "./70.jpg"
import img71 from "./71.jpg"
import img72 from "./72.jpg"
import img73 from "./73.jpg"
import img74 from "./74.jpg"
import img75 from "./75.jpg"
import img76 from "./76.jpg"
import img77 from "./77.jpg"
import img78 from "./78.jpg"
import img79 from "./79.jpg"
import img80 from "./80.jpg"
import img81 from "./81.jpg"
import img82 from "./82.jpg"
import img83 from "./83.jpg"
import img84 from "./84.jpg"
import img85 from "./85.jpg"
import img86 from "./86.jpg"
import img87 from "./87.jpg"
import img88 from "./88.jpg"
import img89 from "./89.jpg"
import img90 from "./90.jpg"
import img91 from "./91.jpg"
import img92 from "./92.jpg"
import img93 from "./93.jpg"
import img94 from "./94.jpg"
import bin from "./bin_icon.png"
import cart from "./cart_icon.png"
import cross from "./cross_icon.png"
import drop from "./dropdown_icon.png"
import razor from "./razorpay_logo.png"
import profile from "./profile_icon.png"
import search from "./search_icon.png"
import menu from "./menu_icon.png"
import hero from "./hero.png"
import about from "./about.png"
import contact from "./contactImg.jpeg"

export const assets = {
    bin,
    cart,
    cross,
    drop,
    search,
    profile,
    menu,
    hero,
    about,
    razor,
    contact
}

export const products = [
    {
        _id: "aaaaa",
        name: "Lux Cozi Men's Cotton Regular Fit Polo T-Shirt",
        description: "Material - 60% Cotton and 40% Polyester, Half sleeve Polo T-Shirt",
        price: 349,
        image: [img1,img2,img3,img4],
        category: "Men",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: true
    },
    {
        _id: "aaaab",
        name: "Amazon Brand - Symbol Men's Cotton Blend Regular Fit Polo T-Shirt",
        description: "The half sleeves offer a relaxed and versatile style, perfect for warmer weather or layering under jackets.",
        price: 449,
        image: [img5,img6,img7,img8],
        category: "Men",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: true
    },
    {
        _id: "aaaac",
        name: "Louis Philippe Men's Cotton Slim Fit T-Shirt",
        description: "Complement your everyday style with this Crew Neck T-shirt from Louis Philippe by House of Louis Philippe.",
        price: 699,
        image: [img9,img10,img11,img12],
        category: "Men",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: false
    },
    {
        _id: "aaaad",
        name: "Jockey Men's Cotton Regular Fit T-Shirt",
        description: "Super combed Cotton Rich fabric. Ribbed round neck to prevent sagging. Regular fit. Label free for all day comfort.",
        price: 579,
        image: [img13,img14,img15,img16],
        category: "Men",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: false
    },
    {
        _id: "aaaae",
        name: "Amazon Brand - Symbol Men's Cotton Regular Fit Full Sleeve Shirt",
        description: "Soft and Breathable 100% Cotton Fabric, Mandarin collar solid color shirt, Single pocket at chest, Regular fit long sleeve shirt",
        price: 549,
        image: [img17,img18,img19,img20],
        category: "Men",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: false
    },
    {
        _id: "aaaaf",
        name: "DAMENSCH Men's Cotton Blend Regular Fit Button Down Shirt ",
        price: 1493,
        description: "Earth’s most loved: Where the world sees trash, we saw a chance to turn trash into fashion. This shirt for men, has recycled polyester made from plastic bottles disposed in the oceans.",
        image: [img21,img22,img23,img24],
        category: "Men",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: true
    },
    {
        _id: "aaaag",
        name: "Allen Solly Men's Cotton Slim Fit Shirt",
        price: 869,
        description: "Allen solly Mens Cotton Full sleeve Overdyed shirts with contrast logo",
        image: [img25,img26,img27,img28],
        category: "Men",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: true
    },
    {
        _id: "aaaah",
        name: "Men's Cotton Slim Fit Casual Shirt",
        price: 749,
        description: "Dennis Lingo Men's Cotton Solid Casual Shirt",
        image: [img29,img30,img31],
        category: "Men",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: true
    },
    {
        _id: "aaaai",
        name: "Louis Philippe Super Slim Fit All Day Comfort, Premium Twill Jeans",
        price: 1699,
        description: "The perfect companion for every occasion, this pair of Jeans by Louis Philippe is a must have for you.",
        image: [img32,img33,img34,img35],
        category: "Men",
        subCategory: "Bottomwear",
        sizes: ["28", "30", "32", "34"],
        date: Date.now(),
        bestSeller: false
    },
    {
        _id: "aaaaaj",
        name: "Ben Martin Men's Denim Cotton Oversized Loose Baggy Fit Jeans Pants for Men",
        price: 749,
        description: "Introducing Ben Martin's Men's Baggy Jeans (Loose, Oversized, Boyfriend Jeans For Men). Designed for modern comfort and style.",
        image: [img36,img37,img38,img39],
        category: "Men",
        subCategory: "Bottomwear",
        sizes: ["28", "30", "32", "34"],
        date: Date.now(),
        bestSeller: true
    },
    {
        _id: "aaaaak",
        name: "TAGDO Men Regular Fit Jeans",
        description: "Tagdo jeans redefine men's fashion with unparalleled comfort, timeless style, and impeccable quality.",
        price: 699,
        image: [img40,img41,img42,img43],
        category: "Men",
        subCategory: "Bottomwear",
        sizes: ["28", "30", "32", "34"],
        date: Date.now(),
        bestSeller: true
    },
    {
        _id: "aaaaal",
        name: "JVX Jeans for Men || Men Jeans || Looses Fit DenimJeans || BaggyJeans",
        description: "This Baggy Fit Jeans for Men 1) Comfortable for all-day wear. 2) Casual and laid-back style. 3) side pockets for added functionality. 4) available in different washes and finishes.",
        price: 799,
        image: [img44,img45,img46,img47],
        category: "Men",
        subCategory: "Bottomwear",
        sizes: ["28", "30", "32", "34"],
        date: Date.now(),
        bestSeller: false
    },
    {
        _id: "aaaaam",
        name: "Fabricorn Women's Cotton Regular Fit T-Shirt",
        description: "Shop from a wide range of T-Shirt from Fabricorn. T-shirt fashion meticulously crafted with 100% Cotton fabric.",
        price: 399,
        image: [img48,img49,img50,img51],
        category: "Women",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: true
    },
    {
        _id: "aaaaan",
        name: "Max Women Printed Round Neck Regular Fit T-Shirt",
        description: "MAX Top",
        price: 269,
        image: [img52,img53,img54,img55],
        category: "Women",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: true
    },
    {
        _id: "aaaaao",
        name: "London Hills Women's Cotton Blend Oversized Fit T-Shirt",
        description: "Sleeve Type: Half Sleeve.",
        price: 242,
        image: [img56,img57,img58],
        category: "Women",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: false
    },
    {
        _id: "aaaaap",
        name: "KOTTY Women's Solid Slim Fit T-Shirt",
        description: "Classic Round Neck, Stretchable Cotton Tee",
        price: 329,
        image: [img59,img60,img61,img62],
        category: "Women",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: true
    },
    {
        _id: "aaaaaq",
        name: "GRECIILOOKS Women's Rayon Regular Fit Top",
        description: "This oversized white shirt for women includes a women's shirt in comfortable Poly Viscose textured fabric is used to make this women shirt.",
        price: 399,
        image: [img63,img64,img65,img66],
        category: "Women",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: true
    },
    {
        _id: "aaaaar",
        name: "ROYALICA Women's Regular Fit Red Heart Printed Button Down Collar Casual White Shirt",
        description: "This women's printed button-down shirt is the epitome of chic sophistication, offering a perfect blend of elegance and style",
        price: 389,
        image: [img67,img68,img69,img70],
        category: "Women",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: false
    },
    {
        _id: "aaaaas",
        name: "FUNDAY FASHION Women's Crepe Regular Fit Shirt",
        description: "Trendy and appealing, this shirt from FUNDAY FASHION exclusive collection will surely impress fashionable women",
        price:284,
        image: [img71,img72,img73,img74],
        category: "Women",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: true
    },
    {
        _id: "aaaaat",
        name: "GRECIILOOKS Women's CREPE Regular Fit Shirt",
        description: "Refresh your Wardrobe With our versatile collection of Women's shirts , designed to meet every style need.",
        price: 449,
        image: [img75,img76,img77,img78],
        category: "Women",
        subCategory: "Topwear",
        sizes: ["S", "M", "L", "XL"],
        date: Date.now(),
        bestSeller: true
    },
    {
        _id: "aaaaau",
        name: "Symbol Premium Women's Straight fit Jeans ",
        description: "Symbol Premium brings you timeless elevated styles that transition from ‘desk to dinner’ seamlessly.",
        price: 1999,
        image: [img79,img80,img81,img82],
        category: "Women",
        subCategory: "Bottomwear",
        sizes: ["28", "30", "32", "34"],
        date: Date.now(),
        bestSeller: true
    },
    {
        _id: "aaaaav",
        name: "Nifty Women's Cotton Blend Stretchable Mid Rise Jeans",
        description: "Nifty brings you comfortable bottom wears ranging from jeans, jeggings, shorts, capris, & dungarees.",
        price: 482,
        image: [img83,img84,img85,img86],
        category: "Women",
        subCategory: "Bottomwear",
        sizes: ["28", "30", "32", "34"],
        date: Date.now(),
        bestSeller: false
    },
    {
        _id: "aaaaaw",
        name: "Ben Martin Jeans for Women",
        description: " High Waist Jeans for Women || Bootcut Jeans for Women || Flared Jeans for Women High Waist || Wide Leg Jeans Women || Bell Bottom Jeans for Women",
        price: 749,
        image: [img87,img88,img89,img90],
        category: "Women",
        subCategory: "Bottomwear",
        sizes: ["28", "30", "32", "34"],
        date: Date.now(),
        bestSeller: false
    },
    {
        _id: "aaaaax",
        name: "GRECIILOOKS Jeans for Women ",
        description: "Bootcut Jeans for Women High Waist | Women Jeans | Baggy Jeans | High Waist | Bell Bottom Jeans",
        price: 699,
        image: [img91,img92,img93,img94],
        category: "Women",
        subCategory: "Bottomwear",
        sizes: ["28", "30", "32", "34"],
        date: Date.now(),
        bestSeller: true
    }
    
    

]